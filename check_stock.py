#!/usr/bin/env python3
"""
Script to check current stock availability for an item
"""
import frappe
...
def get_current_stock(item_code):
    ...
    # Get the latest end_qty for each location/handling_unit/batch/serial combination
    sql = """
        SELECT
            l.storage_location,
            l.handling_unit,
    ...
    print(f"{'-'*80}")
    print(f"{'Location':<30} {'Handling Unit':<20} {'Batch':<15} {'Serial':<15} {'Qty':<10} {'Last Transaction':<20}")
    print(f"{'-'*80}")
    ...
    for row in results:
        ...
        print(f"{location:<30} {hu:<20} {batch:<15} {serial:<15} {qty:<10.3f} {last_date:<20}")
...
if __name__ == "__main__":
    ...
    item_code = "EPH-ACC-002-ELCTWRLDPH"
    results, total = get_current_stock(item_code)
