...
def get_data(filters):
	...
	pending = frappe.db.sql("""
		...
		FROM `tabDeclaration` d
		WHERE d.docstatus != 2
		AND d.status IN ('Draft', 'Lodged', 'Assessment on Hold', 'PAN Issued', 'FAN Issued', 'Paid')
		{conditions}
	""".format(conditions=conditions), filters, as_dict=1)[0]
	...
	overdue = frappe.db.sql("""
		...
		FROM `tabDeclaration` d
		WHERE d.docstatus != 2
		AND d.status NOT IN ('Released', 'Rejected', 'Cancelled')
	...
	avg_days = frappe.db.sql("""
		...
		FROM `tabDeclaration` d
		WHERE d.docstatus != 2
		AND d.status IN ('Released', 'Rejected')
		{conditions}
	""".format(conditions=conditions), filters, as_dict=1)[0]
