...
| Vessel tracking notice (`integration_settings_html`) | HTML | **Purpose:** Reminds users that vessel-tracking providers, API keys, cache TTL and fallback order are configured site-wide in **GoConnect Settings ▸ Vessel** — no longer on this form. **What to enter:** Read-only notice. |
