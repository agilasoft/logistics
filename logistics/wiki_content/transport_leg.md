...
| Pick Window Start (`pick_window_start`) | Time | **Purpose:** Clock time for shifts, gate hours, or cut-off times without a full date. **What to enter:** Time only (HH:MM or per ERPNext control). **Behaviour:** Filled from the Address **Pick / Drop Windows** schedule for the pick weekday when address or pick date changes. Verify after edits; you may override for this leg. |
| Pick Window End (`pick_window_end`) | Time | **Purpose:** Clock time for shifts, gate hours, or cut-off times without a full date. **What to enter:** Time only (HH:MM or per ERPNext control). **Behaviour:** Filled from the Address **Pick / Drop Windows** schedule for the pick weekday when address or pick date changes. Verify after edits; you may override for this leg. |
| Pick Date and Time (`pick_datetime`) | Datetime | **Purpose:** Scheduled pickup appointment for this leg (date and time). **What to enter:** Pick date and time; use the time zone your process expects (often local site). **Behaviour:** Copied from Transport Order Leg when the Transport Job is created. Distinct from pick window times and POD signed-at. |
...
| Drop Window Start (`drop_window_start`) | Time | **Purpose:** Clock time for shifts, gate hours, or cut-off times without a full date. **What to enter:** Time only (HH:MM or per ERPNext control). **Behaviour:** Filled from the Address **Pick / Drop Windows** schedule for the drop weekday when address or drop date changes. Verify after edits; you may override for this leg. |
| Drop Window End (`drop_window_end`) | Time | **Purpose:** Clock time for shifts, gate hours, or cut-off times without a full date. **What to enter:** Time only (HH:MM or per ERPNext control). **Behaviour:** Filled from the Address **Pick / Drop Windows** schedule for the drop weekday when address or drop date changes. Verify after edits; you may override for this leg. |
| Drop Date and Time (`drop_datetime`) | Datetime | **Purpose:** Scheduled delivery appointment for this leg (date and time). **What to enter:** Pick date and time; use the time zone your process expects (often local site). **Behaviour:** Copied from Transport Order Leg when the Transport Job is created. Distinct from drop window times and POD signed-at. |
