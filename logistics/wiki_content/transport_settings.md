...
- **Routing Event Map** – Event map on transport forms
...
| Routing Event Map (`routing_show_map`) | Check | **Purpose:** Boolean flag that drives validation, billing, DG handling, or UI (depending on the field label). **What to enter:** Tick **Yes** / enabled, untick **No** / disabled. |
...
| Warn on Missing Vehicle Average Speed (`warn_on_missing_vehicle_avg_speed`) | Check | **From definition:** Event warning when vehicle doesn't have avg_speed set **Purpose:** Boolean flag that drives validation, billing, DG handling, or UI (depending on the field label). **What to enter:** Tick **Yes** / enabled, untick **No** / disabled. |
| Warn on Missing Cargo Volume (`warn_on_missing_cargo_volume`) | Check | **From definition:** Event warning when cargo volume cannot be calculated for loading/unloading time **Purpose:** Boolean flag that drives validation, billing, DG handling, or UI (depending on the field label). **What to enter:** Tick **Yes** / enabled, untick **No** / disabled. |
