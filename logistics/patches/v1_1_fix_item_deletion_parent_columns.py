...
def execute():
	...
	for table_name in tables_to_fix:
		try:
			...
			# Use backticks for table name comparison
			...
			if existing_columns == required_columns:
				print(f"  ✓ {table_name}: All parent columns already exist")
				skipped_count += 1
				continue
			...
			if alter_statements:
				...
				print(f"  ✓ {table_name}: Added columns {', '.join(missing)}")
				fixed_count += 1
			else:
				# Only missing index
				if index_exists[0]['count'] == 0:
					frappe.db.sql(f"ALTER TABLE `{table_name}` ADD INDEX `parent` (`parent`)")
					print(f"  ✓ {table_name}: Added parent index")
					fixed_count += 1
				else:
	...
	print(f"\n✅ Patch completed: {fixed_count} table(s) fixed, {skipped_count} table(s) skipped")
