...
"""Install Netting operations dashboard Custom HTML Block and refresh the workspace."""
...
BLOCK_NAME = "Netting Operations Dashboard"
WORKSPACE_NAME = "Netting"
...
def _sync_workspace():
	...
	workspace_path = os.path.join(app_path, "netting", "workspace", "netting", "netting.json")
	...
	if frappe.db.exists("Workspace", WORKSPACE_NAME):
		...
		doc.module = data.get("module") or "Netting"
		doc.public = 1
	...
	doc.type = data.get("type") or doc.type or "Workspace"
	...
	if roles:
		_replace_children(doc, "roles", roles)
	doc.flags.ignore_validate = True
...
def _sync_sidebar():
	sidebar_path = os.path.join(frappe.get_app_path("logistics"), "workspace_sidebar", "netting.json")
	if os.path.isfile(sidebar_path):
		import_file_by_path(sidebar_path, force=True)
