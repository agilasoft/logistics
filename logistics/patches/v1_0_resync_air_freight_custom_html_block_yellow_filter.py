...
# Re-sync Air Freight Operations Custom HTML Block (yellow-only map + traffic filter overlay).
...
def execute():
	...
	if updated:
		frappe.db.commit()
		print(f"  Re-synced {updated} Custom HTML Block(s) (yellow map filter).")
