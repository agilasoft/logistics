...
# Re-sync Air Freight Operations block: company title, no breadcrumb/map label row, ring = shipment count, no shipment cap UI.
...
def execute():
	...
	if updated:
		frappe.db.commit()
		print(f"  Re-synced {updated} Custom HTML Block(s) (ops header / shipments / no cap).")
