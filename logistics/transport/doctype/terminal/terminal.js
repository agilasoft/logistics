...
frappe.ui.form.on('Terminal', {
  setup(frm) {
    // Primary Contact query → only contacts linked to this Terminal (server-side to avoid contact.link_doctype permission errors)
    frm.set_query('terminal_primary_contact', function (doc) {
      if (doc.__islocal || !doc.name) return { filters: { name: '__none__' } };
      let contact_names = [];
      frappe.call({
        method: 'logistics.contact_links.get_contact_names_for_dynamic_link',
        args: { link_doctype: 'Terminal', link_name: doc.name },
        async: false,
    ...
    frm.set_query('terminal_primary_address', function (doc) {
      if (doc.__islocal || !doc.name) return { filters: { name: '__none__' } };
      return logistics.address.query_for_link('Terminal', doc.name);
    });
  },

  // Keep a formatted preview of the selected primary address (same as Customer)
  terminal_primary_address(frm) {
    if (frm.doc.terminal_primary_address) {
      frappe.call({
        method: 'frappe.contacts.doctype.address.address.get_address_display',
        args: {
          // Terminal passes the name; method accepts a name or dict
          address_dict: frm.doc.terminal_primary_address,
        },
        callback: function (r) {
  ...
  terminal_primary_contact(frm) {
    if (!frm.doc.terminal_primary_contact) {
      // Add these fields to Terminal if you want them synced
