...
frappe.provide("logistics.transport_operations");
...
frappe.pages["transport-operations"].on_page_load = function (wrapper) {
  logistics.transport_operations.page = new logistics.transport_operations.TransportOperationsPage(wrapper);
};

(function () {
  class TransportOperationsPage {
    constructor(wrapper) {
        ...
        <div class="af-ops-dashboard">
          <div class="af-ops-toolbar form-inline">
            ...
            <span class="af-ops-filter-group af-ops-filter-group-airlines">
              <span class="af-ops-filter-lbl">${__("Transport companies")}</span>
    ...
    _bind_airline_dropdown() {
      ...
      if (!logistics.transport_operations._trOpsCarrierDocClose) {
        logistics.transport_operations._trOpsCarrierDocClose = true;
        $(document).on("click.trOpsCarrierDd", (e) => {
    ...
    _update_airline_summary() {
      ...
      if (!n) {
        $sum.text(__("No transport companies"));
        return;
      }
      if (!selN || selN === n) {
        $sum.text(__("All transport companies"));
        return;
      }
      if (selN === 1) {
    ...
    _render_alerts(summary, items) {
      ...
      items.forEach((it) => {
        ...
        const link = frappe.utils.get_form_link("Transport Job", it.shipment);
        const row =
  ...
  logistics.transport_operations.TransportOperationsPage = TransportOperationsPage;
})();
