...
(function() {
	...
	function build_charges_table(charges, company) {
		...
		var rows = (charges || []).map(function(c, i) {
			...
			return [
				...
				'<td class="text-right">' + (c.revenue != null ? (typeof frappe.format === "function" ? frappe.format(c.revenue, { fieldtype: "Currency", options: cur || company }) : c.revenue.toFixed(2)) : "") + '</td>',
	...
	function fetch_exchange_rate(posting_date, from_currency, to_currency, callback) {
	...
	function fetch_customer_billing_currency(customer, company_currency, callback) {
		if (!customer) {
			if (callback) callback(company_currency);
			return;
		}
		frappe.db.get_value("Customer", customer, "default_currency", function(r) {
			var cur = (r && r.message && r.message.default_currency) || company_currency;
			if (callback) callback(cur || company_currency);
		});
	}

	function bind_select_all(dialog) {
	...
	window.show_create_sales_invoice_dialog = function(frm) {
		...
		function refresh_exchange_rate_field() {
			...
			if (!billing_currency || billing_currency === company_currency) {
				...
				if ($rate) $rate.df.read_only = 1;
				if ($rate) $rate.refresh();
				return;
			}
			if ($rate) $rate.df.read_only = 0;
			if ($rate) $rate.refresh();
			fetch_exchange_rate(posting_date, billing_currency, company_currency, function(rate) {
				if (dialog) dialog.set_value("exchange_rate", rate);
			});
		}

		function set_billing_currency_from_customer(customer) {
			fetch_customer_billing_currency(customer, company_currency, function(cur) {
				if (dialog) {
		...
		fetch_charges(null, null, function(data) {
			...
			company_currency = data.company_currency || company;
			var default_billing_currency = data.default_billing_currency || company_currency;
			...
			function build_dialog_with_charges(chargeData) {
				...
				dialog.$wrapper.find("[data-fieldname='customer']").on("change", function() {
					var customer = dialog.get_value("customer");
					set_billing_currency_from_customer(customer);
					refetch_and_refresh();
				});
