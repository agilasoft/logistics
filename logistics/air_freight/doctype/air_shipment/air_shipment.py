...
def booking_milestone_row_values(src):
	"""Child-row dict copied from an Air Booking milestone onto Air Shipment."""
...
class AirShipment(VirtualLinkedServicesMixin, Document):
	...
	def get_milestone_html(self):
		...
		try:
			# Document alerts at top of dashboard
	...
	def get_dashboard_html(self):
		...
		try:
			from logistics.document_management.logistics_form_dashboard import (
				build_air_shipment_dashboard_config,
			...
			dash = render_logistics_form_dashboard_html(
				self, build_air_shipment_dashboard_config(self)
			)
	...
	def _get_linked_air_booking(self):
		booking_name = (getattr(self, "air_booking", None) or "").strip()
		if not booking_name or not frappe.db.exists("Air Booking", booking_name):
			return None
		return frappe.get_doc("Air Booking", booking_name)

	def _guard_from_booking_milestone_edits(self):
		"""Reject client edits/deletes of booking-copied milestone rows still on the booking."""
		if getattr(frappe.flags, "in_import", False) or getattr(frappe.flags, "in_migrate", False):
			return
		if self.is_new() or not self.name:
			return
		booking = self._get_linked_air_booking()
		booking_milestones = {
		...
		persisted = frappe.get_all(
			...
			filters={"parent": self.name, "parenttype": "Air Shipment", "from_booking": 1},
		...
		for old in persisted:
			...
			if not incoming:
				if still_on_booking:
					frappe.throw(_("Milestones copied from Air Booking cannot be deleted."))
				continue
			if still_on_booking and _booking_milestone_fields_changed(old, incoming):
				frappe.throw(_("Milestones copied from Air Booking cannot be edited."))

	def sync_milestones_from_air_booking(self):
		"""Populate and refresh Air Booking milestone rows; leave shipment-only rows editable."""
		if getattr(frappe.flags, "in_import", False) or getattr(frappe.flags, "in_migrate", False):
			return
	...
	def _iter_linked_service_milestone_sources(self):
		"""Order and Job docs for Linked Services on this shipment (and its Air Booking)."""
		...
		skip_docs = {("Air Shipment", (self.name or "").strip())}
		booking_name = (getattr(self, "air_booking", None) or "").strip()
		if booking_name:
			skip_docs.add(("Air Booking", booking_name))

		ls_names = []
		seen_ls = set()
		parents = [("Air Shipment", self.name)]
		if booking_name:
			parents.append(("Air Booking", booking_name))
	...
	def _guard_from_service_milestone_edits(self):
		...
		persisted = frappe.get_all(
			"Air Shipment Milestone",
			filters={"parent": self.name, "parenttype": "Air Shipment", "from_service": 1},
			fields=[
				"name",
				"milestone",
				"planned_start",
...
@frappe.whitelist()
def air_shipment_exists(docname):
	"""Return True if the Air Shipment exists. Used by client to poll before navigating so form load does not show 'not found'."""
	if not docname or docname == "new":
		return False
	return bool(frappe.db.exists("Air Shipment", docname))
