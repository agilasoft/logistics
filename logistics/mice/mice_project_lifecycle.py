...
def validate_lifecycle_stage_advance(doc):
	_validate_lifecycle_stage_advance(
		doc,
		settings_doctype="MICE Settings",
		module_filter=FOR_EXHIBITS,
	)


def create_orders_for_event(event_name):
	if not frappe.db.get_single_value("MICE Settings", "auto_create_phase_orders"):
		return
	...
	program = frappe.get_doc("MICE Project", event_name)
	for stage in get_open_lifecycle_stages(FOR_EXHIBITS):
		...
		order.status = "Draft"
		# MICE Project no longer carries a customer link directly; the billing
		# Customer is resolved via the linked MICE Organizer (when set).
		getter = getattr(program, "get_organizer_customer", None)
		order.customer = getter() if callable(getter) else None
