...
function _set_programme_charge_qty_read_only(frm) {
...
frappe.ui.form.on("Special Project", {
	refresh: function (frm) {
		...
		_set_programme_charge_qty_read_only(frm);
	},
});

function _fetch_sales_quote_link_from_change_request(cdt, cdn) {
	var row = locals[cdt] && locals[cdt][cdn];
	if (!row || !row.change_request || row.sales_quote_link) {
		return;
	}
	frappe.db.get_value("Change Request", row.change_request, "sales_quote", function (r) {
		if (r && r.sales_quote) {
			frappe.model.set_value(cdt, cdn, "sales_quote_link", r.sales_quote);
		}
	});
}

frappe.ui.form.on("Special Project Charges", {
	change_request: function (frm, cdt, cdn) {
		_fetch_sales_quote_link_from_change_request(cdt, cdn);
	},
