...
class SpecialProjectCharges(Document):
    """Special Project Charges (Special Project) child table - uses centralized charge calculation."""

    def validate(self):
        validate_charge_item_not_manual_other_service(self, "Special Project Charges", "charge_item")
    ...
    def _set_container_deposit_pending_refund_gl_display(self):
        ...
        if not item or not item_is_container_deposit(item):
            ...
            return
        company = None
        if self.get("parent") and self.get("parenttype") == "Special Project":
            company = frappe.db.get_value("Special Project", self.parent, "company")
        self.container_deposit_pending_refund_gl = (
