...
"""Site materials ledger and programme cargo helpers for Special Project."""
...
def _posted_receipts(sp_doc: Any) -> list[Any]:
	return [
		r
		for r in (getattr(sp_doc, "site_receipts", None) or [])
		if (getattr(r, "status", None) or POSTED_RECEIPT_STATUS) == POSTED_RECEIPT_STATUS
		and flt(getattr(r, "qty_received", 0)) > 0
	]


def sync_site_material_balances(sp_doc: Any) -> None:
	"""Recompute qty_on_site and qty_short on each site_materials row from posted receipts.

	Rows where include_on_create is ticked are off-ledger always-along packages and stay
	at qty_on_site = qty_short = 0.
	"""
	materials = getattr(sp_doc, "site_materials", None) or []
	if not materials:
		return
	...
	for rc in _posted_receipts(sp_doc):
		qty = flt(getattr(rc, "qty_received", 0))
		row_idx = cint_safe(getattr(rc, "site_material_row", None))
		if row_idx:
	...
	for mat in materials:
		if cint_safe(getattr(mat, "include_on_create", 0)):
			...
			mat.qty_short = 0
			continue
		idx = cint_safe(getattr(mat, "idx", None))
		on_site = flt(totals_by_row.get(idx, 0))
		if not on_site:
			wh = _norm(getattr(mat, "warehouse_item", None))
			if wh:
				on_site = flt(totals_by_warehouse_item.get(wh, 0))
			if not on_site:
				commodity = _norm(getattr(mat, "commodity", None))
				if commodity:
					on_site = flt(totals_by_commodity.get(commodity, 0))
			if not on_site:
				desc = _norm_desc(getattr(mat, "description", None))
				if desc:
					on_site = flt(totals_by_desc.get(desc, 0))
		required = flt(getattr(mat, "qty_required", 0))
		mat.qty_on_site = on_site
		mat.qty_short = max(required - on_site, 0)


def validate_site_materials(sp_doc: Any) -> None:
	"""Validate site material lines and receipt idempotency keys."""
	materials = getattr(sp_doc, "site_materials", None) or []
	for i, mat in enumerate(materials, start=1):
		wh = _norm(getattr(mat, "warehouse_item", None))
		commodity = _norm(getattr(mat, "commodity", None))
		desc = _norm(getattr(mat, "description", None))
		if not wh and not commodity and not desc:
			frappe.throw(
				_("Site Materials row {0}: enter Warehouse Item, Commodity, or Description.").format(i),
				title=_("Site Materials"),
			)
		if flt(getattr(mat, "qty_required", 0)) <= 0:
			frappe.throw(
				_("Site Materials row {0}: Qty Required must be greater than zero.").format(i),
				title=_("Site Materials"),
			)

	seen_idem: set[tuple[str, str, int]] = set()
	for j, rc in enumerate(getattr(sp_doc, "site_receipts", None) or [], start=1):
		...
		if src_dt and src_name:
			...
			if key in seen_idem:
				frappe.throw(
					_("Site Receipts row {0}: duplicate source {1} {2} package {3}.").format(
	...
	sync_site_material_balances(sp_doc)

	for mat in materials:
		if cint_safe(getattr(mat, "include_on_create", 0)):
			continue
		if flt(getattr(mat, "qty_on_site", 0)) > flt(getattr(mat, "qty_required", 0)):
			label = site_material_label(mat)
			frappe.msgprint(
				_("Site material {0}: on-site quantity exceeds required quantity.").format(label),
				indicator="orange",
				title=_("Site Materials"),
			)


def site_material_label(mat: Any) -> str:
	wh = _norm(getattr(mat, "warehouse_item", None))
	if wh:
	...
	if commodity:
		return commodity
	return _norm(getattr(mat, "description", None)) or _("line")
...
def _find_site_material_row(
	...
	*,
	...
	for mat in getattr(sp_doc, "site_materials", None) or []:
		if warehouse_item and _norm(getattr(mat, "warehouse_item", None)) == warehouse_item:
			return mat
		if commodity and _norm(getattr(mat, "commodity", None)) == commodity:
			return mat
		if description and not warehouse_item and not commodity:
			if _norm(getattr(mat, "description", None)).lower() == description.lower():
				return mat
	return None
...
def seed_site_materials_from_sales_quote(sp_doc: Any, sales_quote: Any, *, clear_existing: bool = False) -> int:
	"""Copy Sales Quote project_products into site_materials requirement rows."""
	products = getattr(sales_quote, "project_products", None) or []
	if not products:
		return 0
	if clear_existing:
		sp_doc.set("site_materials", [])

	existing_wh = {
		_norm(getattr(m, "warehouse_item", None))
		for m in (getattr(sp_doc, "site_materials", None) or [])
...
_SITE_MATERIAL_PACKAGE_FIELDS: tuple[str, ...] = (
	"hs_code",
	"reference_no",
	"no_of_packs",
	"length",
...
def _site_material_row_dict(sp_doc: Any, row_idx: int) -> dict[str, Any] | None:
	materials = getattr(sp_doc, "site_materials", None) or []
	if row_idx < 1 or row_idx > len(materials):
		return None
	mat = materials[row_idx - 1]
	out: dict[str, Any] = {
		"site_material_row": row_idx,
		"commodity": getattr(mat, "commodity", None),
		"warehouse_item": getattr(mat, "warehouse_item", None),
		"description": getattr(mat, "description", None),
		"uom": getattr(mat, "uom", None),
	}
	for fn in _SITE_MATERIAL_PACKAGE_FIELDS:
		out[fn] = getattr(mat, fn, None)
	return out
...
def apply_shipment_lines_to_target(
	...
	"""Append package rows from Booking/Order dialog site material picks."""
	...
	for line in lines:
		...
		if qty <= 0:
			continue
		row_idx = cint_safe(line.get("site_material_row"))
		mat_info = _site_material_row_dict(sp_doc, row_idx) if row_idx else {}
		mat_info = mat_info or {}
		wh = _norm(line.get("warehouse_item") or mat_info.get("warehouse_item"))
		commodity = _norm(line.get("commodity") or mat_info.get("commodity"))
		desc = _norm(line.get("description") or mat_info.get("description"))
...
def copy_always_along_site_materials_to_target(sp_doc: Any, target_doc: Any) -> int:
	"""Append always-along site material rows (include_on_create=1) onto target packages table.

	These rows are hidden from the Shipment Lines dialog, so there is no dedupe to perform.
	"""
	materials = getattr(sp_doc, "site_materials", None) or []
	rows = [m for m in materials if cint_safe(getattr(m, "include_on_create", 0))]
	if not rows:
		return 0
	child_dt = _child_doctype_for_table_field(target_doc.doctype, "packages")
	if not child_dt:
	...
	count = 0
	for mat in rows:
		qty = flt(getattr(mat, "no_of_packs", 0)) or flt(getattr(mat, "qty_required", 0))
		if qty <= 0:
			qty = 1
		row_dict: dict[str, Any] = {}
		if meta.get_field("quantity"):
		...
		for fn in ("warehouse_item", "commodity", "description", "uom"):
			value = getattr(mat, fn, None)
			if value and meta.get_field(fn):
				row_dict[fn] = value
		wh = _norm(getattr(mat, "warehouse_item", None))
		if not getattr(mat, "description", None) and wh and meta.get_field("description"):
			row_dict["description"] = (
				frappe.db.get_value("Warehouse Item", wh, "item_name") or wh
			)
		for fn in _SITE_MATERIAL_PACKAGE_FIELDS:
...
def _receipt_exists(sp_doc: Any, source_doctype: str, source_name: str, package_idx: int) -> bool:
	for rc in getattr(sp_doc, "site_receipts", None) or []:
		if (
...
def build_receipts_from_transport_order(tro: Any) -> list[dict[str, Any]]:
	"""Build site receipt row dicts from a submitted Transport Order (caller persists on SP)."""
...
def post_site_receipts_from_transport_order(tro: Any) -> int:
	...
	if n:
		frappe.msgprint(
			_("Posted {0} site receipt line(s) to Special Project {1}.").format(n, sp_name),
			indicator="green",
			title=_("Site Materials"),
		)
	return n
...
def cancel_receipts_for_transport_order(tro: Any) -> int:
	...
	sp = frappe.get_doc("Special Project", sp_name)
	changed = 0
	for rc in getattr(sp, "site_receipts", None) or []:
		if (
...
def build_receipts_from_project_doc(doc: Any) -> list[dict[str, Any]]:
	"""Build site receipt row dicts from a Project Order / Project Job ``materials_received`` table.
	...
	for row in getattr(doc, "materials_received", None) or []:
		...
		desc = _norm(getattr(row, "description", None))
		explicit_row_link = cint_safe(getattr(row, "site_material_row", None))
		mat = None
		if explicit_row_link:
			materials = getattr(sp_doc, "site_materials", None) or []
			if 1 <= explicit_row_link <= len(materials):
				mat = materials[explicit_row_link - 1]
		if mat is None:
			mat = _find_site_material_row(
				sp_doc, warehouse_item=wh, commodity=commodity, description=desc
			)
...
def post_site_receipts_from_project_doc(doc: Any) -> int:
	"""Append site receipts on the parent Special Project when a Project Order / Project Job is submitted."""
...
def cancel_receipts_for_project_doc(doc: Any) -> int:
	"""Flip matching posted site receipts to ``Cancelled`` when a Project Order / Project Job is cancelled."""
	sp_name = _resolve_sp_from_project_doc(doc)
	if not sp_name:
		return 0
	sp = frappe.get_doc("Special Project", sp_name)
...
@frappe.whitelist()
def recalculate_site_material_balances(special_project: str) -> dict[str, Any]:
	doc = frappe.get_doc("Special Project", special_project)
	doc.check_permission("write")
	validate_site_materials(doc)
	doc.save(ignore_permissions=True)
	return {"ok": True, "message": _("Site material balances updated.")}


@frappe.whitelist()
def get_site_materials_for_shipment_picker(special_project: str) -> list[dict[str, Any]]:
	"""Return requirement rows for Booking/Order shipment lines dialog."""
	...
	validate_site_materials(doc)
	out: list[dict[str, Any]] = []
	for mat in getattr(doc, "site_materials", None) or []:
		if cint_safe(getattr(mat, "include_on_create", 0)):
			continue
		wh = _norm(getattr(mat, "warehouse_item", None))
		...
		out.append(
			{
				"site_material_row": cint_safe(getattr(mat, "idx", None)),
				"commodity": getattr(mat, "commodity", None),
				"warehouse_item": wh or None,
				"warehouse_item_name": warehouse_item_name,
				"description": getattr(mat, "description", None),
				"qty_required": flt(getattr(mat, "qty_required", 0)),
				"qty_on_site": flt(getattr(mat, "qty_on_site", 0)),
				"qty_short": flt(getattr(mat, "qty_short", 0)),
				"uom": getattr(mat, "uom", None),
			}
		)
	return out
...
@frappe.whitelist()
def add_site_materials_to_transport_order(
	special_project: str,
	shipment_lines: Any,
	lifecycle_job_idx: int | None = None,
	lifecycle_jobs: Any = None,
) -> dict[str, Any]:
	"""Create Transport Order from lifecycle row with selected shipment lines as packages."""
