// route: sea-freight-operations
// title: Sea Freight Operations

frappe.provide("logistics.sea_freight_operations");
...
frappe.pages["sea-freight-operations"].on_page_load = function (wrapper) {
  logistics.sea_freight_operations.page = new logistics.sea_freight_operations.SeaFreightOperationsPage(wrapper);
};

(function () {
  class SeaFreightOperationsPage {
    constructor(wrapper) {
      this.wrapper = $(wrapper);
      this.page = frappe.ui.make_app_page({
        parent: wrapper,
        title: __("Sea Freight Operations"),
        single_column: true,
      });
      this.map = null;
      ...
      const $ui = $(`
        <div class="af-ops-dashboard">
          <div class="af-ops-toolbar form-inline">
            ...
            <span class="af-ops-filter-group af-ops-filter-group-airlines">
              <span class="af-ops-filter-lbl">${__("Shipping lines")}</span>
              <div id="af-ops-airline-ms" class="af-ops-airline-ms">
                <button type="button" id="af-ops-airline-dd-toggle" class="btn btn-default btn-xs form-control text-left af-ops-airline-dd-toggle" aria-expanded="false" aria-haspopup="listbox">
                  <span id="af-ops-airline-summary" class="af-ops-airline-summary"></span>
                </button>
                <div class="af-ops-airline-dd-panel" role="listbox">
                  <div id="af-ops-airline-checkboxes" class="af-ops-airline-checkboxes"></div>
                </div>
              </div>
            </span>
          </div>
    ...
    _bind_airline_dropdown() {
      if (this._airline_dd_bound) return;
      this._airline_dd_bound = true;
      const $root = this.page.main;
      $root.on("click", "#af-ops-airline-dd-toggle", (e) => {
        e.stopPropagation();
        const $w = $root.find("#af-ops-airline-ms");
        const open = !$w.hasClass("af-ops-airline-dd-open");
        $w.toggleClass("af-ops-airline-dd-open", open);
        $(e.currentTarget).attr("aria-expanded", open ? "true" : "false");
      });
      if (!logistics.sea_freight_operations._sfOpsCarrierDocClose) {
        logistics.sea_freight_operations._sfOpsCarrierDocClose = true;
        $(document).on("click.sfOpsCarrierDd", (e) => {
          $(".af-ops-airline-ms.af-ops-airline-dd-open").each(function () {
            if (this.contains(e.target)) return;
            $(this).removeClass("af-ops-airline-dd-open");
            $(this).find(".af-ops-airline-dd-toggle").attr("aria-expanded", "false");
          });
        });
      }
      $root.on("change", ".af-ops-airline-cb", () => {
    ...
    _update_airline_summary() {
      ...
      if (!n) {
        $sum.text(__("No shipping lines"));
        return;
      }
      if (!selN || selN === n) {
        $sum.text(__("All shipping lines"));
        return;
      }
      if (selN === 1) {
    ...
    _render_alerts(summary, items) {
      ...
      items.forEach((it) => {
        ...
        const link = frappe.utils.get_form_link("Sea Shipment", it.shipment);
        const row =
      ...
      let groupsHtml = "";
      const MAX_ALERT_ROWS_PER_GROUP = 10;
      order.forEach((level) => {
        const cnt = counts[level];
        if (!cnt) return;
        const label = (labels[level] || "").replace("%s", String(cnt));
        const rowHtmls = groups[level];
        const shown = rowHtmls.slice(0, MAX_ALERT_ROWS_PER_GROUP);
        let bodyInner = shown.join("");
        const restCount = rowHtmls.length - shown.length;
        if (restCount > 0) {
          bodyInner +=
  ...
  logistics.sea_freight_operations.SeaFreightOperationsPage = SeaFreightOperationsPage;
})();
