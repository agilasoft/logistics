...
_BOOKING_MILESTONE_GUARD_FIELDS = (
    "milestone",
    "planned_start",
    "planned_end",
    "actual_start",
...
def booking_milestone_row_values(src):
    """Child-row dict copied from a Sea Booking milestone onto Sea Shipment."""
    values = {fn: getattr(src, fn, None) for fn in BOOKING_MILESTONE_COPY_FIELDS}
    values["from_booking"] = 1
    return values


def _apply_booking_milestone_values(dest, src):
    for fn in BOOKING_MILESTONE_COPY_FIELDS:
...
def _milestone_guard_fields_changed(old_row, incoming_row):
    for fn in _BOOKING_MILESTONE_GUARD_FIELDS:
        old_val = _normalize_milestone_compare_value(fn, old_row.get(fn) if isinstance(old_row, dict) else getattr(old_row, fn, None))
...
def _booking_milestone_fields_changed(old_row, incoming_row):
...
def refresh_service_milestones_on_sea_shipment(name):
    """Persist linked-service milestone copy onto a Sea Shipment without a client save."""
    shipment_name = (name or "").strip()
    if not shipment_name or not frappe.db.exists("Sea Shipment", shipment_name):
        return
    if getattr(frappe.flags, "in_sea_shipment_service_milestone_push", False):
        return
    frappe.flags.in_sea_shipment_service_milestone_push = True
    try:
        doc = frappe.get_doc("Sea Shipment", shipment_name)
        doc.flags.ignore_documents_milestones_populate = True
        doc.sync_milestones_from_linked_services()
        doc.save(ignore_permissions=True)
    finally:
        frappe.flags.in_sea_shipment_service_milestone_push = False


def refresh_service_milestones_after_internal_job_link(parent_doctype, parent_name, linked_service=None):
    """Copy Order/Job milestones onto related Sea Shipments after a satellite job is linked."""
    names = set()
    parent_dt = (parent_doctype or "").strip()
    parent_nm = (parent_name or "").strip()
    if parent_dt == "Sea Shipment" and parent_nm:
        names.add(parent_nm)
    elif parent_dt == "Sea Booking" and parent_nm:
        names.update(
            frappe.get_all("Sea Shipment", filters={"sea_booking": parent_nm}, pluck="name")
        )
    else:
        ls = (linked_service or "").strip()
        if ls:
            from logistics.utils.linked_service_usage import get_usages_for_linked_service

            for usage in get_usages_for_linked_service(ls):
                if (usage.get("used_on_doctype") or "").strip() != "Sea Shipment":
                    continue
                used_name = (usage.get("used_on_name") or "").strip()
                if used_name:
                    names.add(used_name)
    for shipment_name in names:
        refresh_service_milestones_on_sea_shipment(shipment_name)
...
class SeaShipment(VirtualLinkedServicesMixin, Document):
    ...
    def _get_linked_sea_booking(self):
        booking_name = (getattr(self, "sea_booking", None) or "").strip()
        if not booking_name or not frappe.db.exists("Sea Booking", booking_name):
            return None
        return frappe.get_doc("Sea Booking", booking_name)

    def _guard_from_booking_milestone_edits(self):
        """Reject client edits/deletes of booking-copied milestone rows that are still on the booking."""
        if getattr(frappe.flags, "in_import", False) or getattr(frappe.flags, "in_migrate", False):
            return
        if self.is_new() or not self.name:
            return
        booking = self._get_linked_sea_booking()
        booking_milestones = {
            (row.milestone or "").strip()
            for row in ((booking.get("milestones") if booking else None) or [])
            if (row.milestone or "").strip()
        }
        persisted = frappe.get_all(
            "Sea Shipment Milestone",
            filters={"parent": self.name, "parenttype": "Sea Shipment", "from_booking": 1},
        ...
        for old in persisted:
            still_on_booking = (old.milestone or "").strip() in booking_milestones
            incoming = incoming_by_name.get(old.name)
            if not incoming:
                if still_on_booking:
                    frappe.throw(_("Milestones copied from Sea Booking cannot be deleted."))
                continue
            if still_on_booking and _booking_milestone_fields_changed(old, incoming):
                frappe.throw(_("Milestones copied from Sea Booking cannot be edited."))

    def sync_milestones_from_sea_booking(self):
        """Populate and refresh Sea Booking milestone rows; leave shipment-only rows editable."""
        if getattr(frappe.flags, "in_import", False) or getattr(frappe.flags, "in_migrate", False):
            return
        ...
        claimed = set()
        for booking_row in booking.get("milestones") or []:
            milestone = (getattr(booking_row, "milestone", None) or "").strip()
            if not milestone:
                continue
            dest = None
            unclaimed = [row for row in existing_rows if id(row) not in claimed and (row.milestone or "").strip() == milestone]
            ...
            _apply_booking_milestone_values(dest, booking_row)
            claimed.add(id(dest))
    ...
    def _iter_linked_service_milestone_sources(self):
        """Order and Job docs for Linked Services on this shipment (and its Sea Booking)."""
        ...
        skip_docs = {("Sea Shipment", (self.name or "").strip())}
        booking_name = (getattr(self, "sea_booking", None) or "").strip()
        if booking_name:
            skip_docs.add(("Sea Booking", booking_name))

        ls_names = []
        seen_ls = set()
        parents = [("Sea Shipment", self.name)]
        if booking_name:
            parents.append(("Sea Booking", booking_name))
    ...
    def _guard_from_service_milestone_edits(self):
        ...
        persisted = frappe.get_all(
            "Sea Shipment Milestone",
            filters={"parent": self.name, "parenttype": "Sea Shipment", "from_service": 1},
            fields=[
                "name",
                "milestone",
                "planned_start",
    ...
    def sync_milestones_from_linked_services(self):
        """Populate and refresh linked-service Order/Job milestone rows; leave shipment-only rows editable."""
        if getattr(frappe.flags, "in_import", False) or getattr(frappe.flags, "in_migrate", False):
            return
