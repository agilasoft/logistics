...
frappe.ui.form.on("Sea Consolidation Plan", {
	refresh(frm) {
		if (frm.doc.docstatus === 0 && !frm.is_new()) {
			frm.add_custom_button(__("Fetch matching shipments"), () => {
				frappe.call({
					method: "fetch_matching_sea_shipments",
					doc: frm.doc,
					freeze: true,
					callback(r) {
						if (r.exc) {
		...
		if (frm.doc.docstatus === 1) {
			const all_linked = (frm.doc.items || []).every((r) => r.linked_sea_consolidation);
			if (!all_linked) {
				frm.add_custom_button(__("Create Sea Consolidation"), () => {
					frappe.call({
						method: "create_sea_consolidation_from_plan",
						doc: frm.doc,
						freeze: true,
						callback(r) {
							if (!r.exc && r.message) {
								frappe.set_route("Form", "Sea Consolidation", r.message);
							}
						},
					});
				});
