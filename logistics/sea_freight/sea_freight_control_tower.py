...
- Top shipping lines (limit is a per-user preference)
...
PREFS_KEY = "sfct_preferences"
DEFAULT_SHIPPING_LINE_LIMIT = 10
MAX_SHIPPING_LINE_LIMIT = 50
...
DEFAULT_PREFERENCES = {
	...
	"shipping_lines": {
		"limit": DEFAULT_SHIPPING_LINE_LIMIT,
	},
	"returned": {
		"visible": 1,
	},
...
def _clamp_shipping_line_limit(n):
	n = cint(n) or DEFAULT_SHIPPING_LINE_LIMIT
	return max(1, min(MAX_SHIPPING_LINE_LIMIT, n))
...
def _merge_preferences(raw=None):
	...
	for section, defaults in DEFAULT_PREFERENCES.items():
		...
		out[section] = merged
	out["shipping_lines"]["limit"] = _clamp_shipping_line_limit(out["shipping_lines"].get("limit"))
...
def _sea_shipment_kpis(filters):
	"""Open / avg age / handled counts for Sea Shipment."""
	today = nowdate()
	...
	try:
		...
		frappe.log_error(frappe.get_traceback(), "sfct_open_jobs")
	...
	try:
		rs = frappe.db.sql(
			"""
			SELECT COUNT(*) FROM `tabSea Shipment`
			WHERE {where}
		...
		frappe.log_error(frappe.get_traceback(), "sfct_handled_jobs")
...
def _avg_lead_time(filters):
	"""Average actual vs planned milestone days on Sea Shipments matching filters."""
	...
	try:
		rs = frappe.db.sql(
			...
			tuple([SEA_SHIPMENT] + extra_values),
		)
		...
		frappe.log_error(frappe.get_traceback(), "sfct_avg_lead_time")
		return 0.0
...
def _top_shipping_lines(filters, n=DEFAULT_SHIPPING_LINE_LIMIT):
	from_date, to_date = _date_bounds(filters)
	dim_c, dim_v = _dim_clauses(filters)
	unloco_c, unloco_v = _unloco_clause(filters)
	conditions = dim_c + unloco_c + [
		"booking_date BETWEEN %s AND %s",
		"shipping_line IS NOT NULL",
		"shipping_line != ''",
	]
	values = dim_v + unloco_v + [from_date, to_date]
	limit = _clamp_shipping_line_limit(n)
	try:
		rs = frappe.db.sql(
			"""
			SELECT shipping_line AS label, COUNT(*) AS value
			FROM `tabSea Shipment`
			WHERE {where}
		...
		frappe.log_error(frappe.get_traceback(), "sfct_top_shipping_lines")
		rs = []
...
def _returned_billings_count(filters):
	...
	# Prefer Sea Freight module rows when the column exists.
	try:
		if frappe.db.has_column("Returned Billing", "module"):
			conditions.append("(module = %s OR IFNULL(module, '') = '')")
			values.append("Sea Freight")
	except Exception:
		pass

	# UNLOCO is not on Returned Billing — when set, restrict via linked Sea Shipment.
	unloco = filters.get("unloco")
	try:
		if unloco:
			conditions.append(
				"""(
					IFNULL(job_no, '') != ''
					AND EXISTS (
						SELECT 1 FROM `tabSea Shipment` s
		...
		frappe.log_error(frappe.get_traceback(), "sfct_returned_billings")
		return 0


@frappe.whitelist()
def get_preferences():
	"""Per-user Sea Freight Control Tower widget preferences."""
...
def save_preferences(preferences=None):
	"""Persist per-user widget preferences for Sea Freight Control Tower."""
...
def get_dashboard_data(filters=None, company=None, branch=None, cost_center=None,
		profit_center=None, unloco=None, fiscal_year=None, from_date=None, to_date=None,
		shipping_line_limit=None):
	"""Return all Sea Freight Control Tower metrics in one call."""
	f = _parse_filters(
		filters=filters,
		company=company,
		branch=branch,
	...
	limit = _clamp_shipping_line_limit(
		shipping_line_limit if shipping_line_limit not in (None, "") else prefs["shipping_lines"]["limit"]
	)

	kpi = _sea_shipment_kpis(f)
	lead_time = _avg_lead_time(f)
	shipping_lines = _top_shipping_lines(f, n=limit)
	returned = _returned_billings_count(f)

	top_shipping_lines = []
	for row in shipping_lines:
		top_shipping_lines.append({
			"label": row.get("label") or _("Unknown"),
			"value": flt(row.get("value") or 0),
		})
	max_shipping_line = max([r["value"] for r in top_shipping_lines], default=0) or 1
	...
	return {
		...
		"top_shipping_lines_max": max_shipping_line,
		"by_module": kpi.get("by_module") or [],
		"links": {
			"job_files_open": "SFCT Job Files Detail",
			"job_files_handled": "SFCT Job Files Detail",
			"milestone_lead_time": "SFCT Milestone Lead Time",
			"shipping_line_volumes": "SFCT Shipping Line Volumes",
			"returned_billings": "SFCT Returned Billings",
		},
	}
...
def get_filter_options(company=None):
	...
	UNLOCO options are distinct origin/destination ports used on Sea Shipments
	...
	try:
		...
		rows = frappe.db.sql(
			...
			SELECT DISTINCT port FROM (
				SELECT origin_port AS port FROM `tabSea Shipment` WHERE {where}
				UNION
				SELECT destination_port AS port FROM `tabSea Shipment` WHERE {where}
			) u
			WHERE IFNULL(port, '') != ''
			ORDER BY port
			LIMIT 500
		...
		frappe.log_error(frappe.get_traceback(), "sfct_unloco_options")
