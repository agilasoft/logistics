...
class ExhibitCharges(Document):
    ...
    def validate(self):
        validate_charge_item_not_manual_other_service(self, "Exhibit Charges", "charge_item")
    ...
    def _resolve_parent_company(self):
        ...
        if not parent or not parenttype:
            return None
        if parenttype == "Exhibit":
            return frappe.db.get_value("Exhibit", parent, "company")
        if parenttype == "Docket":
            ...
            if exhibit:
                return frappe.db.get_value("Exhibit", exhibit, "company")
        if parenttype == "Exhibit Order":
            company = frappe.db.get_value("Exhibit Order", parent, "company")
            if company:
                return company
            exhibit = frappe.db.get_value("Exhibit Order", parent, "exhibit")
            if exhibit:
                return frappe.db.get_value("Exhibit", exhibit, "company")
        if parenttype == "Exhibit Job":
            company = frappe.db.get_value("Exhibit Job", parent, "company")
            if company:
                return company
            exhibit = frappe.db.get_value("Exhibit Job", parent, "exhibit")
            if exhibit:
                return frappe.db.get_value("Exhibit", exhibit, "company")
        return None
