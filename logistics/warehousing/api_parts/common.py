...
def _query_available_candidates(
    ...
    params: List[Any] = [
        # latest subquery WHERE (lines 683-685)
        item, batch_no, batch_no, serial_no, serial_no,
        # latest subquery join (line 693)
        item,
        # main join (line 701)
        item,
        # first_seen subquery (lines 710-712)
        item, batch_no, batch_no, serial_no, serial_no,
        # main WHERE (lines 723, 725-726)
        item, batch_no, batch_no, serial_no, serial_no,
    ]
...
def create_sales_invoice_from_periodic_billing(
    ...
    # Set company, branch, profit_center, cost_center, and job_number on header if fields exist
    ...
    # Append items (also set branch, profit_center, cost_center, and job_number per row if fields exist)
    ...
    item_has_job_number = "job_number" in sif_item_fields
    ...
    for r in valid_rows:
        ...
        if job_number and item_has_job_number: row_payload["job_number"] = job_number
