...
class WarehouseJob(Document):
	...
	def _get_milestone_status(self, handling_unit_name, job_type):
		"""Get milestone status for a handling unit based on posted operations in job items."""
		try:
			milestones = {
				'start': False,
				'received': False,
			...
			if not self.items or not handling_unit_name:
				return milestones

			# Aggregate status from all item rows for this handling unit
			received_any = putaway_any = pick_any = release_any = False
			for it in self.items:
				hu = getattr(it, "handling_unit", None)
				if hu != handling_unit_name:
					continue
				received_any = received_any or getattr(it, "staging_posted", 0) or getattr(it, "receiving_posted", 0) or getattr(it, "putaway_posted", 0)
				putaway_any = putaway_any or getattr(it, "putaway_posted", 0)
				pick_any = pick_any or getattr(it, "pick_posted", 0)
				release_any = release_any or getattr(it, "release_posted", 0) or pick_any  # release uses pick for outbound

			milestones['start'] = True  # Job exists
			milestones['received'] = bool(received_any)
			milestones['putaway'] = bool(putaway_any)
			milestones['picked'] = bool(pick_any)
			milestones['released'] = bool(release_any)
			return milestones
	...
	def _get_milestone_details(self, handling_unit_name, job_type):
		"""Get milestone details (location and date) for a handling unit from job item posting history."""
		try:
			details = {
			...
			for it in self.items:
				...
				if hu != handling_unit_name:
					continue
				# Received: staging area and staging_posted_at
				if getattr(it, "staging_posted", 0) or getattr(it, "receiving_posted", 0):
					loc = getattr(it, "staging_area", None) or getattr(it, "location", None)
					dt = getattr(it, "staging_posted_at", None) or getattr(it, "receiving_posted_at", None)
					if details['received']['location'] == 'Pending':
						details['received'] = {'location': loc or 'Staging', 'date': str(dt)[:19] if dt else ''}
				# Putaway: destination location and putaway_posted_at
				if getattr(it, "putaway_posted", 0):
					loc = getattr(it, "location", None)
					dt = getattr(it, "putaway_posted_at", None)
					if details['putaway']['location'] == 'Pending':
						details['putaway'] = {'location': loc or 'Storage', 'date': str(dt)[:19] if dt else ''}
				# Picked: location and pick_posted_at
				if getattr(it, "pick_posted", 0):
					loc = getattr(it, "location", None)
					dt = getattr(it, "pick_posted_at", None)
					if details['picked']['location'] == 'Pending':
						details['picked'] = {'location': loc or 'Picked', 'date': str(dt)[:19] if dt else ''}
				# Released: staging/release uses pick_posted
				if getattr(it, "release_posted", 0) or getattr(it, "pick_posted", 0):
					dt = getattr(it, "release_posted_at", None) or getattr(it, "pick_posted_at", None)
					if details['released']['location'] == 'Pending':
						details['released'] = {'location': 'Released', 'date': str(dt)[:19] if dt else ''}
			return details
