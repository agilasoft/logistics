...
frappe.ui.form.on("General Job", {
	...
	refresh(frm) {
		...
		if (!frm.is_new() && !frm.doc.job_number) {
			frm.add_custom_button(__("Create Job Number"), function() {
				frappe.call({
					method: "logistics.logistics.doctype.general_job.general_job.create_job_number",
					args: {
					...
					callback: function(r) {
						if (r.message) {
							frm.reload_doc();
							frappe.msgprint(__("Job Number {0} created successfully", [r.message]));
						}
					}
				});
			}, __("Create"));
