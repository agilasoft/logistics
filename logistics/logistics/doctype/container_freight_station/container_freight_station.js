frappe.ui.form.on('Container Freight Station', {
  setup(frm) {
    // Primary Contact query → only contacts linked to this Container Freight Station (server-side to avoid contact.link_doctype permission errors)
    frm.set_query('cfs_primary_contact', function (doc) {
      ...
      frappe.call({
        ...
        args: { link_doctype: 'Container Freight Station', link_name: doc.name },
        async: false,
    ...
    frm.set_query('cfs_primary_address', function (doc) {
      if (doc.__islocal || !doc.name) return { filters: { name: '__none__' } };
      return logistics.address.query_for_link('Container Freight Station', doc.name);
    });
  },
