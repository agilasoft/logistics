frappe.ui.form.on('Shipper', {
  setup(frm) {
    // Primary Contact query → only contacts linked to this Shipper (server-side to avoid contact.link_doctype permission errors)
    frm.set_query('shipper_primary_contact', function (doc) {
      if (doc.__islocal || !doc.name) return { filters: { name: '__none__' } };
      let contact_names = [];
      frappe.call({
        method: 'logistics.contact_links.get_contact_names_for_dynamic_link',
        args: { link_doctype: 'Shipper', link_name: doc.name },
        async: false,
    ...
    // Primary Address query → only addresses linked to this Shipper
    frm.set_query('shipper_primary_address', function (doc) {
      if (doc.__islocal || !doc.name) return { filters: { name: '__none__' } };
      return { filters: [['Dynamic Link', 'link_doctype', '=', 'Shipper'], ['Dynamic Link', 'link_name', '=', doc.name]] };
    });
  ...
  shipper_primary_address(frm) {
    if (frm.doc.shipper_primary_address) {
      frappe.call({
        method: 'frappe.contacts.doctype.address.address.get_address_display',
        args: {
          // Customer passes the name; method accepts a name or dict
          address_dict: frm.doc.shipper_primary_address,
        },
        callback: function (r) {
  ...
  shipper_primary_contact(frm) {
    if (!frm.doc.shipper_primary_contact) {
      // Add these fields to Shipper if you want them synced
