...
from logistics.utils.charge_service_type import ROUTING_LEG_JOB_TYPES
...
def get_internal_job_detail_for_parent(parent_doc: Any):
	...
	for row in rows:
		if getattr(row, "job_no", None) == my_name and getattr(row, "job_type", None) == my_dt:
			return row

	for row in rows:
		if getattr(row, "job_type", None) not in acceptable:
			continue
...
def get_internal_job_detail_by_service_type(parent_doc: Any, service_type: str):
