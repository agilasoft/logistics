...
"""Replace Project Type masters from the ATN/ASL spreadsheet.

Sheet:
    https://docs.google.com/spreadsheets/d/1t3kAEqS13Ix2PYcyB0kfRi-2oCVDCugtU95qCZQzB9c/edit?gid=2049467057

The document name is the Project Type value (ERPNext autoname field:project_type).
Rows not on the sheet are unlinked, then deleted. Sheet rows are created or updated
and marked active.

Dry run first:

    from logistics.utils.replace_demo_project_types import replace_demo_project_types
    replace_demo_project_types(dry_run=True)

Then run for real (must match the current site name):

    replace_demo_project_types(
        dry_run=False,
...
SHEET_CSV_URL = (
	...
	"/export?format=csv&gid=2049467057"
)

LOCAL_CSV = os.path.join(
	os.path.dirname(os.path.dirname(__file__)),
	"data",
	"atn_asl_project_types.csv",
)


def replace_demo_project_types(dry_run=True, confirm_site=None, source="auto"):
	"""Delete Project Types not in the sheet, then upsert sheet rows.
	...
	if not dry_run and confirm_site != site:
		frappe.throw(
			f"Pass confirm_site='{site}' to replace project types on this site. "
	...
	existing = set(frappe.get_all("Project Type", pluck="name"))
	...
	has_is_active = frappe.db.has_column("Project Type", "is_active")

	print(f"Site: {site}")
	print(f"Mode: {'DRY RUN' if dry_run else 'LIVE'}")
	print(f"Sheet rows: {len(incoming)}")
	print(f"Existing Project Type: {len(existing)}")
	...
	print("\nUnlinking Project Type from other documents...")
	_unlink_project_types(to_delete)
	frappe.db.commit()

	print(f"\nDeleting {len(to_delete)} Project Type records not in the sheet...")
	failed = []
	for name in to_delete:
		try:
			# ERPNext blocks deletion of the built-in "External" type in on_trash.
			frappe.delete_doc(
				"Project Type",
				name,
				force=1,
	...
	print(f"\nUpserting {len(incoming)} Project Type records...")
	created = updated = 0
	for row in incoming:
		try:
			if _upsert_project_type(row, has_is_active):
				created += 1
			else:
				updated += 1
		except Exception as e:
...
def _load_rows(source):
	...
	if source in ("auto", "google"):
		try:
			...
			if "Project Type" in raw.splitlines()[0]:
	...
	frappe.throw("Could not load Project Type sheet. " + " | ".join(errors))
...
def _normalize_rows(rows):
	...
	used = set()
	for raw in rows:
		name = (raw.get("Project Type") or raw.get("project_type") or "").strip()
		description = (raw.get("Description") or raw.get("description") or "").strip() or name
		if not name:
			continue
		if name in used:
			frappe.throw(f"Project Type {name} is repeated on the sheet.")
		used.add(name)
		incoming.append({"name": name, "project_type": name, "description": description})
	return incoming


def _upsert_project_type(row, has_is_active):
	values = {
	...
	if frappe.db.exists("Project Type", row["name"]):
...
def _unlink_project_types(names):
	if not names:
		return

	try:
		link_fields = get_link_fields("Project Type")
	except Exception:
		link_fields = []
	...
	for lf in link_fields:
		parent = lf.parent
		field = lf.fieldname
		if not parent or not field or parent == "Project Type":
			continue
