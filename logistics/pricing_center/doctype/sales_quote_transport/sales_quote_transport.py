...
class SalesQuoteTransport(Document):
    """
    Sales Quote Transport child table for transport rate calculations
    """
    ...
    def validate(self):
        """Validate the sales quote transport record"""
        ...
        try:
            if self.use_tariff_in_revenue and rev_tariff:
                ...
                if not tariff_doc:
                    ...
                    if not matching_rate:
                        frappe.msgprint(_("No matching transport rate found in tariff {0}").format(rev_tariff), alert=True)
                    else:
            ...
            if self.use_tariff_in_cost and cost_tariff:
                ...
                if not tariff_doc:
                    ...
                    matching_rate = self._find_matching_tariff_rate(tariff_doc)
                    if not matching_rate:
                        frappe.msgprint(_("No matching transport rate found in tariff {0}").format(cost_tariff), alert=True)
                    else:
    ...
    def _calculate_per_unit_quantity(self, parent_doc):
        ...
        if self.unit_type == "Distance":
            ...
            return flt(parent_doc.get('total_weight', 0))
        elif self.unit_type == "Volume":
            return flt(parent_doc.get('total_volume', 0))
    ...
    def _get_cost_actual_data(self):
        ...
        return {
            'actual_quantity': flt(self.cost_quantity or 0),
            'actual_weight': flt(parent_doc.get('total_weight', 0)),
            'actual_volume': flt(parent_doc.get('total_volume', 0)),
    ...
    def _get_actual_data_from_parent(self):
        ...
        if parent_doc:
            actual_data.update({
                'actual_weight': flt(parent_doc.get('total_weight', actual_data['actual_weight'])),
                'actual_volume': flt(parent_doc.get('total_volume', actual_data['actual_volume'])),
...
@frappe.whitelist()
def calculate_transport_line(line_data):
    """
    API endpoint to calculate transport line rates
    ...
    try:
        ...
        temp_doc = frappe.new_doc('Sales Quote Transport')
...
@frappe.whitelist()
def get_cost_calculation_methods():
...
@frappe.whitelist()
def get_tariff_rates(tariff_name, item_code=None):
    """
    Get transport rates from a tariff
...
@frappe.whitelist()
def trigger_calculations_for_line(line_data):
    """
    Trigger calculations for a specific line
