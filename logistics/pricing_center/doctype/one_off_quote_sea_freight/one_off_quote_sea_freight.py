...
class OneOffQuoteSeaFreight(Document):
	"""One-Off Quote Sea Freight child table (same structure as Sales Quote Sea Freight)."""
	...
	def validate(self):
		"""Validate the one-off quote sea freight record"""
...
@frappe.whitelist()
def calculate_one_off_sea_freight_line(line_data):
	"""
	API endpoint to calculate one-off quote sea freight line rates
	...
	try:
		...
		temp_doc = frappe.new_doc('One-Off Quote Sea Freight')
		...
		frappe.log_error(f"One-off sea freight line calculation error: {str(e)}")
		return {
			'success': False,
			'error': str(e)
		}
...
@frappe.whitelist()
def trigger_one_off_sea_freight_calculations_for_line(line_data):
	"""
	Trigger calculations for a specific one-off quote sea freight line
