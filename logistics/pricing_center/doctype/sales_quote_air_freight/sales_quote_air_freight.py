...
class SalesQuoteAirFreight(Document):
    ...
    def _prepare_revenue_rate_data(self):
        ...
        return {
            ...
            'minimum_quantity': flt(self.minimum_quantity or 0),
            'minimum_unit_rate': flt(self.minimum_unit_rate or 0),
            ...
            'base_quantity': flt(self.base_quantity or 1),
    ...
    def _prepare_cost_rate_data(self):
        ...
        return {
            ...
            'minimum_unit_rate': flt(self.cost_minimum_unit_rate or 0),
            ...
            'base_quantity': flt(self.cost_base_quantity or 1),
...
@frappe.whitelist()
def calculate_air_freight_line(line_data):
    """
    API endpoint to calculate air freight line rates
...
@frappe.whitelist()
def get_air_freight_calculation_methods():
...
@frappe.whitelist()
def get_air_freight_cost_calculation_methods():
...
@frappe.whitelist()
def get_air_freight_unit_types():
...
@frappe.whitelist()
def get_air_freight_tariff_rates(tariff_name, item_code=None):
    """
    Get air freight rates from a tariff
