"""Render BDO Cheque PDF to a high-DPI PNG for cheque print formats."""
...
OUTPUT_PATH = os.path.join(IMAGES_DIR, "bdo_cheque.png")
SOURCE_CANDIDATES = [
	os.path.join(IMAGES_DIR, "bdo_cheque_source.pdf"),
]
...
def _find_pdf(pdf_path: str | None = None) -> str:
	...
	for candidate in SOURCE_CANDIDATES:
		if os.path.isfile(candidate):
			return candidate
	frappe.throw(
		"BDO Cheque PDF not found. Copy the original PDF to "
		f"{SOURCE_CANDIDATES[0]} or pass pdf_path, then run:\n"
		"bench --site <site> execute "
		"logistics.print_format.payment_entry.generate_bdo_cheque_background.render"
	)


@frappe.whitelist()
def render(pdf_path: str | None = None, dpi: int = 300) -> dict:
	"""Render page 1 of the BDO cheque PDF to a print-quality PNG background."""
	import fitz

	source = _find_pdf(pdf_path)
	os.makedirs(IMAGES_DIR, exist_ok=True)

	archive = os.path.join(IMAGES_DIR, "bdo_cheque_source.pdf")
